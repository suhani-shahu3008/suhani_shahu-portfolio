...
function Mark({ id, path, ellipseEyes }) {
  return (
    <span className="foot-socials__mark">
      <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        <path d={path ?? SOCIAL_ICON_PATHS[id]} />
        {ellipseEyes && (
          <>
...
// Each inner array is one footer row — one social for a full-width row, two
// for a side-by-side pair (matches the original hand-laid-out footer).
const DEFAULT_SOCIALS = [
  [
    { id: 'linkedin', label: 'LinkedIn', url: 'https://www.linkedin.com/in/suhani-shahu-b755463a0/' },
    { id: 'pinterest', label: 'Pinterest', url: 'https://in.pinterest.com/suhanishahu3008/_profile/' },
  ],
  [
    { id: 'behance', label: 'Behance', url: 'https://www.behance.net/suhanishahu' },
    { id: 'snapchat', label: 'Snapchat', url: 'https://www.snapchat.com/@shahu_suhani', ellipseEyes: true },
  ],
  [{ id: 'instagram', label: 'Instagram', url: 'https://www.instagram.com/_softy_3008' }],
  [{ id: 'github', label: 'GitHub', url: 'https://github.com/suhani-shahu3008' }],
  [{ id: 'vercel', label: 'Vercel', url: 'https://vercel.com/suhani-shahu' }],
]

/**
 * Full-bleed multi-column site footer: brand statement, nav links, socials,
 * and contact details. Requires a react-router-dom `<Router>` ancestor (uses
 * `Link` for the nav column).
 *
 * @param {{
 ...
 *   socials?: Array<Array<{ id: string, label: string, url: string, path?: string, ellipseEyes?: boolean }>>,
 *   contactPhone?: string,
...
export default function SiteFooter({
  brandTagline1 = 'I design for people, not just pixels.',
  brandTagline2 = 'Because great design should never stop at the screen.',
  brandTag = 'Designing for screens and beyond',
  navLinks = DEFAULT_NAV_LINKS,
  socials = DEFAULT_SOCIALS,
  contactPhone = '+91 92848 28790',
  contactPhoneHref = 'tel:+919284828790',
  contactEmail = 'suhanishahu3008@gmail.com',
  contactNote = "Always open to exciting projects, collaborations, or just a good conversation about design, technology, and what's next.",
  copyrightText = '© 2026 Suhani Shahu. All rights reserved.',
  roleText = 'UI/UX Designer · Brand Designer · XR/Spatial Designer',
  closingText = '— Till we meet again —',
}) {
  return (
    <footer className="foot" id="site-footer">
      <div className="foot-grid">
        <div className="foot-brand">
          <div>
            ...
            <p className="foot-brand__l1">{brandTagline1}</p>
            <p className="foot-brand__l2">{brandTagline2}</p>
          </div>
        ...
        <div>
          ...
          <ul>
            {navLinks.map(({ to, label }) => (
              <li key={to}><Link to={to} data-cursor>{label}</Link></li>
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="foot-socials">
            {socials.map((row) => (
              <li className={row.length > 1 ? 'foot-socials__row' : undefined} key={row[0].id}>
                {row.map((s) => (
                  <a href={s.url} target="_blank" rel="noopener noreferrer" data-cursor key={s.id}>
                    <Mark id={s.id} path={s.path} ellipseEyes={s.ellipseEyes} />{s.label}<Arrow />
                  </a>
                ))}
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="foot-contact">
            <li>
              <a href={contactPhoneHref} data-cursor>
            ...
            <li>
              <a href={`mailto:${contactEmail}`} data-cursor>
                <span className="foot-contact__icon">
                  <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
          ...
          <p className="foot-contact__note">{contactNote}</p>
        </div>
      </div>
      ...
      <p className="foot-closing">{closingText}</p>
    </footer>
  )
}
